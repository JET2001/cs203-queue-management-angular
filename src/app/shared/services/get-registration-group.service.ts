...
export class GetRegistrationGroupService {
  ...
  getRegGroupOfUser(eventID: number, userID: number | undefined): Promise<RegGroup | undefined> {
    if (eventID == undefined || userID == undefined) return Promise.resolve(undefined);
    return Promise.resolve(this._getRegGroupOfUser(eventID, userID));
  }


  private _getRegGroupOfUser(eventID: number, userID: number): RegGroup | undefined {
