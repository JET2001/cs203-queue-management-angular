...
export class CarouselComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getEventInfoService.loadAllCarousellEvents().subscribe(
      (data: any) => {
  ...
  handleRegisterButtonClick(eventID: string): void {
    ...
    const eventSelected: Event | undefined =
      this.id2CarousellEvent.get(eventID);
    ...
    this.getRegGroupService
      .getRegGroupOfUser(eventID, this.authService.userID)
      .subscribe(
        (data: any) => {
