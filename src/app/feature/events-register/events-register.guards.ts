...
export function eventRegisterGuard(): CanActivateChildFn {
  return () => {
    ...
    if (storeEventInfoService.eventInfo.eventID == undefined) return denyAccess();
...
export function groupRegisterGuard(): CanActivateFn {
  return async() => {
    const authService: AuthenticationService = inject(AuthenticationService);
    const getRegGroupInfoService: GetRegistrationGroupService  = inject(GetRegistrationGroupService);
    const storeEventInfoService: StoreEventInfoService = inject(StoreEventInfoService);
    const storeRegGroupService: StoreRegistrationGroupInfoService = inject(StoreRegistrationGroupInfoService);

    const eventID = storeEventInfoService.eventInfo.eventID;
    const userID = authService.userID;

    let userRegGroup : RegGroup | undefined;
    await getRegGroupInfoService.getRegGroupOfUser(eventID!, userID).then(
      (regGroup : RegGroup | undefined) => userRegGroup = regGroup
    );
    // If user tries to modify group but does not have an actual group, reject
    if (storeRegGroupService.modifyGroup && userRegGroup == undefined) return denyAccess();
